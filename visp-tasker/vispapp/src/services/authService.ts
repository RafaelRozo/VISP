/**
 * VISP - Authentication Service (Expo Go)
 *
 * Handles all authentication-related API calls: login, register, social auth,
 * password reset, and secure token management via expo-secure-store.
 */
...
const STORE_KEY_TOKENS = 'visp_auth_tokens';
const STORE_KEY_USER = 'visp_auth_user';
...
async function storeTokensSecurely(tokens: AuthTokens): Promise<void> {
  await SecureStore.setItemAsync(STORE_KEY_TOKENS, JSON.stringify(tokens));
...
async function getStoredTokens(): Promise<AuthTokens | null> {
  try {
    const value = await SecureStore.getItemAsync(STORE_KEY_TOKENS);
    if (value) {
...
async function getStoredUser(): Promise<User | null> {
  try {
    const value = await SecureStore.getItemAsync(STORE_KEY_USER);
    if (value) {
...
async function clearSecureStorage(): Promise<void> {
  await SecureStore.deleteItemAsync(STORE_KEY_TOKENS);
...
/**
 * Attempt to restore a previously authenticated session from secure store.
