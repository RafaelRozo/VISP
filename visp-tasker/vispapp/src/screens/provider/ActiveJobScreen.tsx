/**
 * VISP - Active Job Screen (Expo Go)
 *
...
export default function ActiveJobScreen(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    if (isEnRoute) {
      ...
      // Watch continuous position updates using expo-location
      (async () => {
        try {
          const sub = await Location.watchPositionAsync(
            ...
            (location) => {
              setProviderLat(location.coords.latitude);
              setProviderLng(location.coords.longitude);
            },
          );
          locationSubRef.current = sub;
        } catch (err) {
          console.warn('[ActiveJob] Watch position error:', err);
        }
      })();
  ...
  useEffect(() => {
    ...
    if (!token) return;
    const coords = `${providerLng},${providerLat};${activeJob.address.longitude},${activeJob.address.latitude}`;
    fetch(
      ...
      .catch((err) => console.warn('[ActiveJob] Route fetch error:', err));
        ...
        <GlassCard variant="dark" style={styles.cardSpacing}>
          ...
          <View style={styles.mapContainer}>
            <MapboxGL.MapView
              ...
              logoEnabled={false}
              accessibilityLabel="Active job navigation map"
            >
              <MapboxGL.Camera
                zoomLevel={14}
                centerCoordinate={[
                  providerLng ?? activeJob.address.longitude,
                  providerLat ?? activeJob.address.latitude,
                ]}
                animationMode="flyTo"
              ...
              {/* Route line */}
              {routeGeoJSON && (
                <MapboxGL.ShapeSource id="active-job-route" shape={routeGeoJSON}>
                  <MapboxGL.LineLayer
