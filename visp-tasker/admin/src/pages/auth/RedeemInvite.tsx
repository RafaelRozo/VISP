...
export default function RedeemInvite() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await adminService.redeemInvite({
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 grid place-items-center px-6 pb-10">
        <form onSubmit={onSubmit} className="glass p-8 w-full max-w-md">
          <h1 className="text-2xl font-bold">{t('redeem.inviteTitle')}</h1>
          <p className="mt-1 text-sm text-textSecondary">
            {t('redeem.inviteSubtitle')}
          </p>
          ...
          >
            {submitting ? t('common.loading') : t('redeem.activate')}
          </button>
